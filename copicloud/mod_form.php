...
class mod_copicloud_mod_form extends moodleform_mod {
    function definition() {
        global $CFG, $DB;
        $mform =& $this->_form;
        ...
        if ($this->current->instance and $this->current->tobemigrated) {
            // copicloud not migrated yet
            $copicloud_old = $DB->get_record('copicloud_old', array('oldid'=>$this->current->instance));
            $mform->addElement('static', 'warning', '', get_string('notmigrated', 'copicloud', $copicloud_old->type));
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'contentsection', get_string('contentheader', 'copicloud'));
        ...
        if (isset($this->current->legacyfiles) and $this->current->legacyfiles != RESOURCELIB_LEGACYFILES_NO) {
            $options = array(RESOURCELIB_LEGACYFILES_DONE   => get_string('legacyfilesdone', 'copicloud'),
                             RESOURCELIB_LEGACYFILES_ACTIVE => get_string('legacyfilesactive', 'copicloud'));
            $mform->addElement('select', 'legacyfiles', get_string('legacyfiles', 'copicloud'), $options);
        }
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'copicloud'), $options);
            $mform->setDefault('display', $config->display);
            $mform->addHelpButton('display', 'displayselect', 'copicloud');
        }

        $mform->addElement('checkbox', 'showsize', get_string('showsize', 'copicloud'));
        $mform->setDefault('showsize', $config->showsize);
        $mform->addHelpButton('showsize', 'showsize', 'copicloud');
        $mform->addElement('checkbox', 'showtype', get_string('showtype', 'copicloud'));
        $mform->setDefault('showtype', $config->showtype);
        $mform->addHelpButton('showtype', 'showtype', 'copicloud');

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'copicloud'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'copicloud'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'copicloud'));
        ...
        $mform->addElement('select', 'filterfiles', get_string('filterfiles', 'copicloud'), $options);
    ...
    function data_preprocessing(&$default_values) {
        if ($this->current->instance and !$this->current->tobemigrated) {
            ...
            file_prepare_draft_area($draftitemid, $this->context->id, 'mod_copicloud', 'content', 0, array('subdirs'=>true));
